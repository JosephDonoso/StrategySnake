...
public class BodySnake {
	...
	private StrategySnakeAttack estrategiaAtaque;
	...
	public BodySnake(float x, float y, Texture tx, World world) {
		...
		estrategiaAtaque = new EasySnakeAttack();
	}
	...
	public int mover(float vel, int dir, SpriteBatch batch) {
		...
		//if(estrategiaAtaque.validarVelAcum(vel, velAcum)) {
			//dir = validarAleatoriaDir(dir);
			dir = estrategiaAtaque.validarAleatoriaDir(dir);
			velAcum = estrategiaAtaque.actualizarVelAcum(velAcum);
			Tail tail, head;
			head = body.get(0);
			...
			float ladoSerpenteo = estrategiaAtaque.validarLadoSerpenteo(stateSerpenteo);
			float posX, posY;
			if(dir == up) { //Up
		...
		if(draw(batch)) { //si está herido que pare de moverse
			dir = stop;
		}
		return dir;
	}
	...
	public void setStrategySnakeAttack(StrategySnakeAttack estrategiaAtaque) {
